...
import (
	...
	"distributed/delayTask/timeWheel/delayQueue"
...
//add添加定时器到时间轮
//如果定时器已过期返回false
func (tw *TimingWheel) add(t *timer.Timer) bool {
...
func (tw *TimingWheel) addOrRun(t *timer.Timer) {
	if !tw.add(t) {
		go t.Task()
	}
}
